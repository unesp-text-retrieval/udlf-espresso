...
comparison of document rankings for the BBC-NEWS dataset.

Dataset: BBC-NEWS (Category-based)
Model: BM25 + CPRR (K=50)
Query: entertainment_094.txt

Usage:
    python scripts/generate_rerank_svg_bbc.py
"""
...
CORPUS_PATH = Path("/Users/luisvenezian/Documents/masters/udlf-text-retrieval/collections-non-beir/bbc-news/trec-format/corpus-and-topics.tsv")
OUTPUT_PATH = Path("/Users/luisvenezian/Documents/masters/udlf-text-espresso/outputs/paper-assets/bbc_rerank_visualization.svg")

QUERY_ID = "entertainment_094.txt"

# Document IDs from the report
ORIGINAL_RANKING = [
    "tech_102.txt",           # #1 - not relevant
    "tech_236.txt",           # #2 - not relevant
    "entertainment_061.txt",  # #3 - relevant
    "entertainment_353.txt",  # #4 - relevant
    "politics_405.txt",       # #5 - not relevant
    "politics_290.txt",       # #6 - not relevant
    "entertainment_085.txt",  # #7 - relevant
    "sport_112.txt",          # #8 - not relevant
    "sport_121.txt",          # #9 - not relevant
    "politics_292.txt",       # #10 - not relevant
]

RERANKED_RANKING = [
    "entertainment_228.txt",  # #1 - relevant
    "entertainment_347.txt",  # #2 - relevant
    "entertainment_353.txt",  # #3 - relevant
    "entertainment_167.txt",  # #4 - relevant
    "entertainment_061.txt",  # #5 - relevant
    "entertainment_085.txt",  # #6 - relevant
    "entertainment_326.txt",  # #7 - relevant
    "entertainment_039.txt",  # #8 - relevant
    "entertainment_294.txt",  # #9 - relevant
    "entertainment_269.txt",  # #10 - relevant
]

ORIGINAL_RELEVANCE = [0, 0, 1, 1, 0, 0, 1, 0, 0, 0]
...
def generate_svg(corpus: dict) -> str:
    ...
    # SVG dimensions
    width = 1200
    height = 1300  # Increased height to avoid legend overlap
...
''')
    ...
    svg_parts.append(f'''
  <!-- Title -->
  <text x="{width//2}" y="35" text-anchor="middle" class="title">UDLF Re-ranking Effect: Category-based Retrieval Improvement</text>
  <text x="{width//2}" y="55" text-anchor="middle" class="subtitle">BBC-NEWS Dataset • BM25 + CPRR (K=50) • Query: entertainment_094.txt</text>
''')
...
''')
  ...
  <rect x="{query_box_x + 25}" y="{query_section_y + 40}" width="120" height="22" fill="{CATEGORY_COLORS['entertainment']}" rx="3"/>
  <text x="{query_box_x + 85}" y="{query_section_y + 56}" text-anchor="middle" class="category">ENTERTAINMENT</text>
''')
...
''')
  ...
  <text x="{left_column_x + column_width//2}" y="{detail_section_y}" text-anchor="middle" class="section-title">Original Retrieval (BM25)</text>
  ...
  <text x="{right_column_x + column_width//2}" y="{detail_section_y}" text-anchor="middle" class="section-title">Re-ranked Results (CPRR K=50)</text>
...
''')
  ...
  <text x="{width - 50}" y="{legend_y + 14}" text-anchor="end" class="metric">Precision@20: <tspan font-weight="bold">20% → 85%</tspan> (+325% improvement)</text>
''')
